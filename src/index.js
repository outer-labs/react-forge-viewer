...
class ForgeViewer extends React.Component {
  ...
  handleViewerInit() {
    ...
    if (this.props.proxy) {
      ...
      Autodesk.Viewing.endpoint.setEndpointAndApi(
        this.props.proxy,
        region !== "us" ? "derivativeV2_EU" : "derivativeV2"
      );
    }
  ...
  handleLoadDocumentSuccess(doc) {
    ...
    let views = doc.getRoot().search({ type: "geometry" }, true);
  ...
  isArrayDifferent(current, next) {
    ...
    else if (current == null || next == null) return true;
    else if (current.length != next.length) return true;
  ...
  shouldComponentUpdateURN(nextProps, nextState) {
    ...
    if (
      !nextProps.urn ||
      nextProps.urn === "" ||
      typeof nextProps.urn === "undefined" ||
  ...
  shouldComponentUpdateView(nextProps, nextState) {
    ...
    if (
      !nextProps.view ||
      typeof nextProps.view === "undefined" ||
